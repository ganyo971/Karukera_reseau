 ...
 */
void do_bind(int sock, const struct sockaddr_in *serv_addr, socklen_t serv_addr_len);	//void do_bind(const int fd, struct sockaddr_in *);

/**
 * accept connections on the supplied file descriptor, filling up the sockaddr structure and the client_fd
 */
int do_accept(int sock, struct sockaddr * addr, socklen_t addr_len);	//void do_accept(const int server_fd, int *client_fd,struct sockaddr * client_addr);
